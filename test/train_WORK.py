...
if __name__ == '__main__':
    ...
    vectorizer = TfidfVectorizer(ngram_range=(1, 2), sublinear_tf=True, lowercase=False)
    train_v = vectorizer.fit_transform(text)

    text_clf = Pipeline([('clf', LogisticRegression(C=4, class_weight='balanced'))])
    parameters = {
        ...
        # 'clf__multi_class': ('ovr', 'auto', 'multinomial'),
        # 'clf__dual': (True, False),
        # 'clf__class_weight': ('balanced', None),
        # 'clf__C': (1, 1.1, 0.9, 0.95, 1.05),
        # 'clf__fit_intercept': (True, False),
        # 'clf__tol': (1e-4, 1.03e-4, 1.1e-4, 1.05e-4, 9.9e-5, 9.95e-5),
        # 'clf__dual': (True, False)
        # 'clf__intercept_scaling': (1, 1.1, 0.9, 0.95, 1.05)
    }
    ...
    # svm = LogisticRegression(C=4,  class_weight='balanced')
